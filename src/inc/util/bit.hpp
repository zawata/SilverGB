...
__force_inline bool fallen(u64 s_old, u64 s_new, u8 bit) {
    return test(s_old, bit) && !test(s_new, bit);
}

__force_inline bool risen(u64 s_old, u64 s_new, u8 bit) {
    return !test(s_old, bit) && test(s_new, bit);
}

__force_inline bool changed(u64 s_old, u64 s_new, u8 bit) {
    return test(s_old, bit) != test(s_new, bit);
}

template<typename T>
__force_inline T sign_extend(T reg, u8 sign_bit) {
...
struct shift_with_carry {
    ...
    static __force_inline T shift_left(T in, u8 shift_amount, bool carry_in, bool *carry_out) {
        if(carry_out != nullptr) {
            if(shift_amount)
                *carry_out = Bit::test(in, bit_count - shift_amount);
            else
                *carry_out = carry_in;
        }
    ...
    static __force_inline T shift_right(T in, u8 shift_amount, bool *carry_out) {
        if(carry_out != nullptr) {
            if(shift_amount)
                *carry_out = Bit::test(in, shift_amount - 1);
            else
                *carry_out = Bit::test(in, bit_count - 1);
        }
        return in >> shift_amount;
    }

    static __force_inline T arithmetic_shift_right(T in, u8 shift_amount, bool *carry_out) {
        if(carry_out != nullptr) {
            if(shift_amount)
                *carry_out = Bit::test(in, shift_amount - 1);
            else
    ...
    static __force_inline T rotate_right(T in, u8 rotate_amt, bool *carry_out) {
        // if we hit this at any ppoint, then we'll need a safer implementation
        assert(rotate_amt < bit_count);
        if(carry_out != nullptr) {
            if(rotate_amt)
                *carry_out = Bit::test(in, rotate_amt - 1);
            else
                *carry_out = Bit::test(in, bit_count - 1);
        }
...
public:
    ...
    BitFieldAccessor &add_field(key_type const &name, T mask) { return add_field(name, mask); }
    ...
    BitFieldAccessor &add_field(key_type const &name, u8 count, u8 offset) { return add_field(name, count, offset); }
