...
constexpr u16 rgb555_to_rgb15(u8 r, u8 g, u8 b) { return r | (((u16)g) << 5) | (((u16)b) << 10); }
...
constexpr PPU::palette_t compat_palettes[] = {
    ...
    { rgb(0x1F,0x1F,0x1F), rgb(0x0F,0x1F,0x06), rgb(0x00,0x10,0x00), rgb(0x00,0x00,0x00) }, // 03 // 30 starts on the last color here
    ...
    { rgb(0x1F,0x1F,0x1F), rgb(0x0B,0x17,0x1F), rgb(0x1F,0x00,0x00), rgb(0x00,0x00,0x1F) }, // 22 // 31 starts on the last color here
    ...
    { rgb(0x00,0x00,0x00), rgb(0x00,0x10,0x10), rgb(0x1F,0x1B,0x00), rgb(0x1F,0x1F,0x1F) }, // 27 // 32 starts on the last color here
...
PPU::PPU(Cartridge *cart, Memory *mem, gb_device_t device, bool bootrom_enabled = false) :
    cart(cart), mem(mem), device(device) {
    ...
    if(!bootrom_enabled) {
        ...
        if(dev_is_GB(device)) {
            ...
            if(cart->isCGBCart()) {
                ...
                if(cart->cartSupportsGBCCompatMode()) {
                    ...
                    if(bounded(compat_plt_id, AMBIGUOUS_CHK_IDXS_START, AMBIGUOUS_CHK_IDXS_END)) {
                        ...
                        const char *found_char
                                = strchr(title_fourth_chars[compat_plt_id - AMBIGUOUS_CHK_IDXS_START], cart_char);
                ...
                case 0b110:
                case 0b111:
                default:    LogWarn("PPU") << "compat: unknown case: " << (shuffle_flags & 0x3);
                }
...
void PPU::write_bg_color_data(u8 data) { set_color_data(&reg(BCPS), bg_palettes, data); }

u8   PPU::read_bg_color_data() { return get_color_data(&reg(BCPS), bg_palettes); }
...
const std::vector<Silver::Pixel> &PPU::getPixelBuffer() { return this->pixBuf; }

bool              PPU::isGBCAllowed() { return dev_is_GBC(this->device) && !mem->get_dmg_compat_mode(); }
